...
MenuDoDraggableRect(rect Rect, v2 P, u32 Priority, u64 ID){
    ...
    RenderRect(TestRect, -3.0f, C, GameItem(1)); 
...
DEBUGRenderSpritesheet(const char *Name, u32 Frame, v2 Center, f32 Z){
    ...
    RenderTexture(R, Z, Asset->Texture, GameItem(1), TextureRect);
    RenderRectOutline(R, -10.0f, RED, GameItem(1));
}
...
DEBUGRenderRect(v2 P){
    ...
    f32  Z = -0.1f;
    color Color = RED;
    RenderQuad(GameRenderer.WhiteTexture, GameItem(1), Z,
...
UpdateAndRenderDebug(){
    ...
    v2 P = V2(40.0f);
    rect R = CenterRect(V2(0), V2(30)) + P;
    RenderRect(R, -10.0f, BLUE, GameItem(1));
