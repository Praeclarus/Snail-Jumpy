...
font_system::Initialize(memory_arena *Arena){
    Fonts = MakeHashTable<string, font>(Arena, 128);
}
...
font_system::LoadFont(string Name, const char *FontPath, f32 Size){
    font *Font = HashTableGetPtr(&Fonts, Name);
    ...
    os_file *File = OSOpenFile(FontPath, OpenFile_Read);
    u64 FileSize = OSGetFileSize(File);
    u8 *FileData = ArenaPushArray(&GlobalTransientMemory, u8, FileSize);
    OSReadFile(File, 0, FileData, FileSize);
    OSCloseFile(File);
    
    u8 *Bitmap;
    BEGIN_TIMED_BLOCK(PushMemory);
    Bitmap = ArenaPushArraySpecial(&GlobalTransientMemory, u8, Width*Height, NoneAndAlign(4));
    END_TIMED_BLOCK();
...
font_system::FindFont(string Name){
    font *Result = HashTableFindPtr(&Fonts, Name);
    return(Result);
}
