...
DoInfoSelector(v2 StartP, string Selected){
 ...
 for(u32 I=0; I<Entities.Count; I++){
  ...
  if(Result == Entities[I]) State->ActiveT += 3*OSInput.dTime; 
...
world_editor::AddWorldEntity(){
 ...
 switch(Mode){
   ...
   *NewEnemy = {};
   ...
   NewEnemy->P = P;
   NewEnemy->EntityInfo = EntityInfoToAdd;
   NewEnemy->Direction = Direction_Left;
   
   NewEnemy->PathStart = P + V2(-0.5f*EntitySize.X, 0.5f*EntitySize.Y);
   NewEnemy->PathStart = SnapToGrid(NewEnemy->PathStart, 1);
   NewEnemy->PathEnd   = P + V2(1.5f*EntitySize.X, 0.5f*EntitySize.Y);
   NewEnemy->PathEnd   = SnapToGrid(NewEnemy->PathEnd, 1);
...
world_editor::DoSelectorOverlay(){
 ...
 switch(Mode){
  case EditMode_AddEnemy: {
   EntityInfoToAdd = DoInfoSelector(P, EntityInfoToAdd);
  }break;
  case EditMode_AddArt: {
