...
internal void
SelectTeleporter(u32 Id){
    Editor.SelectedThingType = EntityType_Teleporter;
    Editor.SelectedThing = Id;
    {
        widget_info *Info = FindOrCreateInHashTablePtr(&UIManager.WidgetTable, "Teleporter Level");
        Info->BufferIndex = CStringLength(Editor.World->Teleporters[Id].Level);
    }{
        widget_info *Info = FindOrCreateInHashTablePtr(&UIManager.WidgetTable, "Teleporter Required Level");
        Info->BufferIndex = CStringLength(Editor.World->Teleporters[Id].RequiredLevel);
    }
}
...
SelectDoor(u32 Id){
    ...
    Editor.SelectedThing = Id;
    {
        widget_info *Info = FindOrCreateInHashTablePtr(&UIManager.WidgetTable, "Door Required Level");
        Info->BufferIndex = CStringLength(Editor.World->Teleporters[Id].Level);
    }
}
       ...
       (Editor.Popup == EditorPopup_RenameLevel)){
        ...
        if(UIButton(RenderGroup, OSInput.WindowSize.Width/2.0f-Width/2.0f, Y, -0.81f,
            ...
            if(Editor.TextInput.Buffer[0] != '\0'){
                world_data *World = FindInHashTablePtr(&WorldTable, (const char *)Editor.TextInput.Buffer);
                if(Editor.Popup == EditorPopup_LoadLevel){
                    if(!World){
                        LoadWorldFromFile(Editor.TextInput.Buffer);
                        World = FindInHashTablePtr(&WorldTable, (const char *)Editor.TextInput.Buffer);
                    }
                    
                    LoadWorld(Editor.TextInput.Buffer);
                    ...
                    if(!World){
                        if(RemoveFromHashTable(&WorldTable, (const char *)CurrentWorld->Name)){
                            CopyCString(CurrentWorld->Name, 
                                        Editor.TextInput.Buffer, 512);
                            Editor.TextInput.Buffer[0] = '\0';
                            Editor.TextInput.BufferIndex = 0;
                            InsertIntoHashTable(&WorldTable, (const char *)CurrentWorld->Name, 
                                                *CurrentWorld);
                        }
                        Editor.Popup = EditorPopup_None;
                    }
...
RenderEditorThingUI(render_group *RenderGroup){
    ...
    switch(Editor.SelectedThingType){
        case EntityType_Snail: {
            ...
            entity_data  *SelectedEnemy = &Editor.World->Enemies[Editor.SelectedThing];
...
UpdateEditor(f32 MetersToPixels){
    ...
    if((Editor.Popup == EditorPopup_None) &&
       !UIManager.HandledInput){
        ...
        local_constant edit_mode ReverseEditModeTable[EditMode_TOTAL] = {
            EditMode_AddDoor, EditMode_None, EditMode_AddWall, EditMode_AddCoinP, 
            EditMode_Snail, EditMode_Sally, EditMode_Dragonfly, EditMode_TOTAL, 
            EditMode_Speedy, EditMode_AddTeleporter, 
        };
        local_constant edit_mode ForwardEditModeTable[EditMode_TOTAL] = {
            EditMode_AddWall, EditMode_AddCoinP, EditMode_Snail, EditMode_Sally, 
            EditMode_Dragonfly, EditMode_Speedy, EditMode_AddTeleporter, EditMode_TOTAL, 
            ...
            if(*TileId == EntityType_Teleporter){
                ...
                if(ClickedOnDoor){
                    ...
                    for(EnemyIndex = 0; 
                        ...
                        entity_data *Enemy = &Editor.World->Enemies[EnemyIndex];
           ...
           (Editor.Mode == EditMode_AddTeleporter)){
            if(IsKeyDown(KeyCode_LeftMouse) && Editor.IsDragging){
                if(*TileId == 0){
                    if(Editor.Mode == EditMode_AddTeleporter){
                        if(!Editor.World->Teleporters.Items){
                            Editor.World->Teleporters = CreateNewArray<teleporter_data>(&TeleporterMemory, 128);
                        }
            ...
            if(IsKeyDown(KeyCode_LeftMouse) && Editor.IsDragging){
                ...
                if((Size.X != 0.0f) && (Size.Y != 0.0f)){
                    if(!Editor.World->Doors.Items){
                        Editor.World->Doors = CreateNewArray<door_data>(&DoorMemory, 128);
                    }
                    ...
                    SelectDoor(Editor.World->Doors.Count-1);
                }else{
                    Assert(0);
                }
            ...
            if(IsKeyJustPressed(KeyCode_LeftMouse) && 
               !DidSelectSomething){
                ...
                if(!Editor.World->Enemies.Items){
                    Editor.World->Enemies = CreateNewArray<entity_data>(&EnemyMemory, 64);
                }
                entity_data *NewEnemy = PushNewArrayItem(&Editor.World->Enemies);
                *NewEnemy = {0};
...
UpdateAndRenderEditor(){
    ...
    if(!Editor.HideUI){
        BeginWindow("Edit world", v2{OSInput.WindowSize.X-400, OSInput.WindowSize.Y-43});
        
        UIText(&RenderGroup, "Current world: %s", Editor.World->Name);
        ...
        local_constant char *ModeTable[EditMode_TOTAL] = {
            "None", "Add wall", "Add coin p", "Snail", "Sally", "Dragonfly", "Speedy", 0,
            "Add teleporter", "Add door",
        };
        ...
        if(_UIButton(&RenderGroup, "Load World")){
            Editor.Popup = EditorPopup_LoadLevel;
            UIManager.SelectedWidgetId = 1;
        }
        ...
        //~ World attributes
        UIText(&RenderGroup, "World style is: %s", ((Editor.World->Flags & WorldFlag_IsTopDown) ? "Top down" : "Platformer"));
        if(_UIButton(&RenderGroup, "Toggle world style")){
            if(Editor.World->Flags & WorldFlag_IsTopDown){
                Editor.World->Flags &= ~WorldFlag_IsTopDown;
            }else{
                Editor.World->Flags |= WorldFlag_IsTopDown;
            }
        }
    ...
    
    {
        ...
        for(u32 I = 0; I < Editor.World->Enemies.Count; I++){
            entity_data *Enemy = &Editor.World->Enemies[I];
